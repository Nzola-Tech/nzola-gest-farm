...
import { SortDescriptor, Table, TableBody, TableCell, TableColumn, TableHeader, TableRow, Selection } from "@heroui/table";
...
export default function ProductsTable() {
    ...
    const { selectedKeys, setSelectedKeys, cart, addToCart, removeFromCart } = useContext(PdvContext)
    ...
    const handleSelectionChange = (keys: Selection) => {
        ...
        if (keys === "all") {
            ...
            const selectedProducts = sortedItems.filter((p) =>
                p.id !== undefined && selectedIds.includes(String(p.id))
            );
    ...
    const topContent = (
        <div className="flex flex-col gap-4">
            ...
            <div className="flex justify-between items-center">
                <span className="text-default-400 text-small">Total {filteredProducts.length} produtos</span>
            </div>
        </div>
    );
    ...
    return (
        ...
        >
            <TableHeader>
                ...
                <TableColumn key="sale_price" allowsSorting>Preço de Venda</TableColumn>
                <TableColumn key="expiration_date" allowsSorting>Data de Validade</TableColumn>
                <TableColumn key="stock_quantity" allowsSorting>Estoque</TableColumn>
            </TableHeader>
            <TableBody
                        ...
                        <TableCell>
                            {(() => {
                                ...
                                if (product.stock_quantity <= 0) {
                                    ...
                                    return (
                                        <span className="text-default-400">{expDate.toLocaleDateString()}</span>
                                    );
                                }
                                ...
                                return (
                                    <span className="text-green-600">{expDate.toLocaleDateString()}</span>
                                );
                            })()}
                        </TableCell>
                        <TableCell>
