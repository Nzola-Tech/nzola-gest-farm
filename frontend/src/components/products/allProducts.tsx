  ...
  const tryDelete = async (id: number) => {
    ...
    if (sales.length > 0) {
      addToast({
        ...
        description:
          "Este produto já foi vendido e não pode ser excluído. Você pode desativá-lo.",
        color: "danger",
        timeout: 12000,
    ...
    <>
      <Table
        ...
        disabledKeys={products
          .filter(
            (p) =>
              (p.deleted && p.id !== undefined) ||
              p.stock_quantity === 0 ||
              p.expiration_date === "Produto expirado",
          )
          .map((p) => p.id!.toString())}
      >
        <TableHeader>
        ...
        <TableBody
          emptyContent={"Adicione Produtos para o sistema"}
          items={items}
        >
          {items.map((product) => {
